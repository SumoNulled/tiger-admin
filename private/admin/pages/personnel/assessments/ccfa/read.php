    ...
    <section class="content">
        <div class="container-fluid">
            ...
            function renderForm($_ARRAY = array(), $error = null, $success = null)
            {
              ...
              $table->set_table_head("Cadet Name, One Mile Run, Ability Group, MSL, Gender");
              ...
              {
                $omr = explode(":", $row['one_mile_run']);
                $seconds = $omr[0] * 60 + $omr[1];
                ...
                $table->add_table_row(
                  array(
                    ...
                    $user->print_name(1),
                    //$row['push_ups'] >= $ccfa->get_pu_min() ? $row['push_ups'] : "<font color='red'>" . $row['push_ups'] . "</font>",
                    //$row['curl_ups'] >= $ccfa->get_cu_min() ? $row['curl_ups'] : "<font color='red'>" . $row['curl_ups'] . "</font>",
                    //$row['one_mile_run'] >= $ccfa->get_omr_min() ? $row['one_mile_run'] : "<font color='red'>" . $row['one_mile_run'] . "</font>",
                    $row['one_mile_run'],
                    $agr,
                    'MS' . $row['msl'],
                    $user->gender()
