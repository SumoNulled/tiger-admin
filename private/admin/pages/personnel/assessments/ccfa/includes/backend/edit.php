...
if (isset($_GET['id']) && !(App\SQL::query("SELECT id FROM `structure_assessments` WHERE `id` = ?", $_GET['id'])->num_rows == 0))
{
    ...
    if (isset($_POST['add_cadet']))
    {
      ...
      foreach($it as $x)
      {
        ...
        $query = "INSERT INTO `personnel_ccfa_scores` (cadet_id, assessment_id) VALUES (?,?)";
        $result = $sql->query($query, array($x[0], $_GET['id']));
      }
    ...
    if (isset($_POST['update_assessments']))
    {
          ...
          foreach($it as $x)
          {
            $x[3] = (date('H', strtotime($x[3])) * 60) + date('i', strtotime($x[3]));
